...
def get_artesian_dict_versioned_monthly(df, column_name):
    anni = df.apply(lambda x: get_value_as_series(x['anno']), axis=1).tolist()
    mesi = df.apply(lambda x: get_value_as_series(x['mese']), axis=1).tolist()
    values = df.apply(lambda x: get_value_as_series(x[column_name]), axis=1).values.tolist()
    dict_of_value_to_send = dict()
    for i in range(len(mesi)):
        giorno_ora = datetime(int(anni[i]), int(mesi[i]), 1, 0)
        dict_of_value_to_send[giorno_ora] = values[i]
    return dict_of_value_to_send
